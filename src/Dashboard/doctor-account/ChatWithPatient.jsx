...
const ChatWithPatient = ({ patientId, patientName, onClose }) => {
    const [messages, setMessages] = useState([]);
    const [newMessage, setNewMessage] = useState('');
    const [loading, setLoading] = useState(true);
    const [sending, setSending] = useState(false);
    const [error, setError] = useState(null);
    // FIX: ref on the scrollable container div
    ...
    const fetchMessages = async () => {
        try {
            const res = await fetch(`${BASE_URL}/messages/doctor/${patientId}`, {
    ...
    // FIX: only the chat container scrolls, not the page
    useEffect(() => {
    ...
    const sendMessage = async (e) => {
        ...
        setSending(true);
        try {
            const res = await fetch(`${BASE_URL}/messages/doctor/send/${patientId}`, {
                method: 'POST',
                headers: {
    ...
    return (
        <div className="flex flex-col h-[450px] border border-gray-200 rounded-xl overflow-hidden shadow-md">
            <div className="bg-primaryColor px-4 py-3 flex items-center justify-between flex-shrink-0">
                <div className="flex items-center gap-2">
                    <div className="w-2 h-2 bg-green-300 rounded-full animate-pulse"></div>
                    <p className="text-white font-semibold text-sm">Chat with {patientName}</p>
                </div>
                {onClose && (
                    <button onClick={onClose} className="text-white opacity-70 hover:opacity-100 text-lg font-bold">×</button>
                )}
            </div>
            ...
            >
                ...
                {messages.map((msg, index) => {
                    const isMe = msg.senderId === doctorId || msg.senderRole === 'doctor';
                    return (
